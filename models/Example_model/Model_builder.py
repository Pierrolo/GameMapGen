...
def full_convs(convs_input ):
    ...
    conv_1 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(convs_input)
    conv_2 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_1)
    conv_3 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_2) #tf.keras.layers.Add()([conv_1, conv_2]))
    conv_4 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_3)
    conv_5 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_4)
    conv_6 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_5)
    conv_7 = tf.keras.layers.Convolution2D(filters = 16, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")(conv_6)
...
def FractalNet(convs_input ):   
    ...
    conv_layer_2 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
    conv_2 = fractal_block(conv_1, conv_layer_2)
    
    conv_layer_3 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
    conv_3 = fractal_block(conv_2, conv_layer_3)
    
    conv_layer_4 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
    conv_4 = fractal_block(conv_3, conv_layer_4)
    
    conv_layer_5 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
    conv_5 = fractal_block(conv_4, conv_layer_5)
    
    conv_layer_6 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
    conv_6 = fractal_block(conv_5, conv_layer_6)
    
    conv_layer_7 = tf.keras.layers.Convolution2D(filters = 32, kernel_size = [2,2], strides = (1,1), activation = "relu", padding = "same")    
...
def get_model_builder(env, args_training):
    ...
    def model_builder(opt = opt):
        ...
        if args_training.separate_pos_and_elem :## add softmax layer
            ...
            conv_max = tf.keras.layers.Convolution2D(filters = 16, kernel_size = [2,2], strides = (2,2), activation = "relu", padding = "same")(conv_max)
            flatten = tf.keras.layers.Flatten()(conv_max)
            dense = flatten
            for neurons in [64,32,16]:
        ...
        if args_training.mask_useless_action : 
            conv_action = Lambda(lambda x: x[0] - x[1]*tf.math.reduce_max(x[0], (1,2,3), keepdims = True))((conv_action, state_reshapped_square_one_hot))
